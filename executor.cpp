...
void executor::on_sock_ready(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		if(!pool->cmd_login("Wms14xqViWwG99R1hLQ1XBN7pdCFGxNuda9PFS74kjLud2SntKWfKE8A1wrSirxAQzVw71nE6r6gPEH9crzDVryP2yjRsxykJ", "x"))
			pool->disconnect();
...
void executor::on_switch_pool(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		...
		const char* dev_pool_addr = jconf::inst()->GetTlsSetting() ? "mine.aeon-pool.com:5555" : "mine.aeon-pool.com:5555";
